...
#merged_data_Spillhaug <- merged_data_Spillhaug %>% filter(!is.na(MEASURED_VALUE_k2))
 ...
 Spillhaug_mean_value_SpIn <- merged_data_Spillhaug %>%
  ...
  dplyr::select(LOCATION, SAMPLE_DATE, SITE_CODE, PARAMETER_ENG, MEASURED_VALUE, MEASURED_UNIT, 
...
evaluate <- evaluate %>%
  dplyr::select(LOCATION, PARAMETER_ENG, MEASURED_VALUE_in,MEASURED_VALUE_out)
...
all_pca2 <- all_pca %>%
  ungroup() %>%
  dplyr::select(diff_MEASURED_VALUE_YJ, diff_pH, TEMPERATURE_VALUE_out, YEAR, PRECIPITATION)
...
all_pca2 <- all_pca %>%
  ungroup() %>%
  dplyr::select(diff_MEASURED_VALUE_log, diff_pH, TEMPERATURE_VALUE_out, YEAR, PRECIPITATION)
...
all_pca2 <- all_pca %>%
  ungroup() %>%
  dplyr::select(diff_MEASURED_VALUE, diff_pH, TEMPERATURE_VALUE_out, YEAR, PRECIPITATION)

all_pca2 <- all_pca2 %>%
  mutate(
     diff_MEASURED_VALUE_log = log(diff_MEASURED_VALUE)) %>%
   dplyr::select(-diff_MEASURED_VALUE)
...
test_median <- test_median %>% group_by(PARAMETER_ENG, LOCATION) %>%
 ...
 test_median <- test_median %>%
   ...
   mutate(
     log10_ug_median_MEASURED_VALUE_in = log10(ug_median_MEASURED_VALUE_in),
     log10_ug_median_MEASURED_VALUE_out = log10(ug_median_MEASURED_VALUE_out),
     log10_ug_sd_MEASURED_VALUE_out = log10(sd_ug_MEASURED_VALUE_out),
     log10_ug_mean_MEASURED_VALUE_out = log10(ug_mean_MEASURED_VALUE_out),
     log10_ug_min_MEASURED_VALUE_out = log10(ug_min_MEASURED_VALUE_out),
     log10_ug_max_MEASURED_VALUE_out = log10(ug_max_MEASURED_VALUE_out)
   ) %>%
   ungroup()
...
"Perfluoroheksan acid (PFHxA)","Ammonium-N", "Iron","Nitrogen","Zinc","Chromium","Nickel", "Acenaphthene", "Benzene", "Fluorene")) %>%
 ...
 mutate(
   log10_median_diff_MEASURED_VALUE = log10(median_diff_MEASURED_VALUE),
   log10_SD_diff_MEASURED_VALUE = log10(SD_diff_MEASURED_VALUE),
   log10_MEAN_diff_MEASURED_VALUE = log10(MEAN_diff_MEASURED_VALUE)) %>%
   ungroup()
...
#To avoid - Inf for sd_MEASURED_VALUE_out if sd = 0 (if < LOQ out).
 test_median <- test_median %>%
   group_by(PARAMETER_ENG, LOCATION) %>%
     mutate(
       log10_ug_sd_MEASURED_VALUE_out = ifelse(is.infinite(log10_ug_sd_MEASURED_VALUE_out), NA, log10_ug_sd_MEASURED_VALUE_out)) %>%
  mutate(
...
ggplot(test_median, aes(
  x = reorder(PARAMETER_ENG, log10_ug_median_MEASURED_VALUE_out),
  y = log10_ug_median_MEASURED_VALUE_out,
  fill = LOCATION
)) +
  geom_point(position = position_dodge(0.8), size = 5, shape = 21, color = "black") +
  geom_errorbar(
    aes(ymin = log10_ug_min_MEASURED_VALUE_out,
        ymax = log10_ug_max_MEASURED_VALUE_out),
    position = position_dodge(0.8), width = 0.2, color = "black", linewidth = 0.8) +
   scale_y_continuous(
     limits = c(-4, NA),
     breaks = seq(-4, 4, by = 2),
  ...
  labs(x = NULL, y = NULL, title = NULL, fill = "Location") +
  theme_minimal(base_size = 14) +
  theme(
...
median$TEMPERATURE_CATEGORY <- cut(median$TEMPERATURE_VALUE, breaks = c(-Inf, 8, Inf), labels = c("Winter", "Summer"))
...
#Merge dfs.
median <- full_join(median_Boelstad, median_Spillhaug, by = c("LOCATION", "YEAR", "PARAMETER_ENG", "SAMPLE_DATE", "SITE_CODE_in", "MEASURED_VALUE_in", "SITE_CODE_out", "MEASURED_VALUE_out", "LOQ_in", "LOQ_out", "TEMPERATURE_CATEGORY_out", "TEMPERATURE_CATEGORY_in"))
...
#Create median values per season and year:
median <- median %>%
  group_by(LOCATION, PARAMETER_ENG) %>%
  mutate(
    MEDIAN_REM_PERCENT = median(REM_EFF_PERCENT)) %>%
  relocate(MEDIAN_REM_PERCENT, .before = TEMPERATURE_CATEGORY_out) %>%
  mutate(TEMPERATURE_CATEGORY = TEMPERATURE_CATEGORY_out) %>%
  ungroup()
...
median <- median %>%
  ...
  relocate(MEDIAN_VALUE_SEASON, .before = MEDIAN_REM_PERCENT) %>% 
  ungroup()

#Count number of samples per season, as well as counts of negative removals (n = ).
counts_per_group <- median %>%
  group_by(LOCATION, PARAMETER_ENG, TEMPERATURE_CATEGORY) %>%
  summarise(
    N_PER_GROUP = n(),
    NEG_COUNT = sum(REM_EFF_PERCENT < 0), 
    .groups = "drop")
...
#Plot acenaphthene, fluorene and ammonium-N:
plot <- test_median %>% filter(PARAMETER_ENG %in% c("Acenaphthene"))

plot %>%
  ggplot(aes(x = diff_pH, y = diff_MEASURED_VALUE, color = LOCATION)) +
  geom_point() +
  geom_smooth(method = "lm") +
    theme_minimal()


#lmer Nickel and Zinc


data_zinc <- a2[a2$PARAMETER_ENG %in% "Zinc", ] #No transformation
data_nickel <- a2[a2$PARAMETER_ENG %in% "Nickel", ] #No transformation




#Scale variables, to make regression possible (due to various scales, e.g. degrees vs. pH)
data_zinc$diff_pH <- scale(data_zinc$diff_pH)
data_zinc$TEMPERATURE_VALUE_out <- scale(data_zinc$TEMPERATURE_VALUE_out)
data_zinc$PRECIPITATION <- scale(data_zinc$PRECIPITATION)
data_zinc$YEAR <- scale(data_zinc$YEAR)

data_nickel$diff_pH <- scale(data_nickel$diff_pH)
data_nickel$TEMPERATURE_VALUE_out <- scale(data_nickel$TEMPERATURE_VALUE_out)
data_nickel$PRECIPITATION <- scale(data_nickel$PRECIPITATION)
data_nickel$YEAR <- scale(data_nickel$YEAR)


#Lmer-models for environmental stressors.
test_lmer_zinc <- lmer(diff_MEASURED_VALUE ~ diff_pH + TEMPERATURE_VALUE_out + YEAR + PRECIPITATION + (1|LOCATION), data = data_zinc)

test_lmer_nickel <- lmer(diff_MEASURED_VALUE ~ diff_pH + TEMPERATURE_VALUE_out + YEAR + PRECIPITATION + (1|LOCATION), data = data_nickel)

#Extract coefficients
fixed_effects_nickel <- coef(summary(test_lmer_zinc))
fixed_effects_zinc <- coef(summary(test_lmer_nickel))


#Create R2.
r.squaredGLMM(test_lmer_iron)[1, "R2m"] #Denne vil være den samme for hele modellen for mangan. Denne vil derfor ikke endres for precipitation eller temperature. Location har mye å si!
#print(r_squared_nickel)

r_squared_zinc <- r.squaredGLMM(test_lmer_zinc)[1, "R2c"] #Denne vil være den samme for hele modellen for mangan. Denne vil derfor ikke endres for precipitation eller temperature. Location har mye å si!
#print(r_squared_zinc)
...
#Extract total variance.
var(predict(test_lmer_acen2, re.form = NA)) + sum(as.data.frame(VarCorr(test_lmer_acen2))$vcov)
